...
async function getClientCredentials(clientId) {
  try {
    ...
    console.log(`Successfully retrieved credentials for client: ${clientId}`);
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (client_id && (!replyioEmail || !replyioPassword)) {
      ...
      replyioEmail = creds.replyioEmail;
      replyioPassword = creds.replyioPassword;
      ...
      if (!replyioEmail || !replyioPassword) {
        return {
          ...
          body: JSON.stringify({
            ...
            client_id: client_id,
            hint: 'Ensure reply_io_user and reply_io_password are stored in the client credentials'
          })
        };
      }
    }
    ...
    if (!replyioEmail || !replyioPassword || !accounts || !Array.isArray(accounts)) {
      const missingFields = [];
      if (!replyioEmail) missingFields.push('replyioEmail');
      if (!replyioPassword) missingFields.push('replyioPassword');
      ...
      return {
        ...
        body: JSON.stringify({ 
          ...
          received: {
            ...
            async: async !== undefined ? `✓ ${async}` : 'optional (not provided)',
            webhookUrl: webhookUrl ? '✓ provided' : 'optional (not provided)'
          },
          expected: {
            replyioEmail: 'string (required) - Reply.io account email',
            replyioPassword: 'string (required) - Reply.io account password',
